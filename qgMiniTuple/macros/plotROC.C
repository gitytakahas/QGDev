...
//    for(TString jetType : {"AK4","AK4chs","AK5","AK5chs"}){
    for(TString jetType : {"AK4chs"}){
      ...
      // Creation of histos: 3 for variables + 4 likelihoods + 4 cdf likelihoods
      ...
      for(int i = 0; i < qgMiniTuple->GetEntries(); ++i){
        ...
        for(int j = 0; j < pt->size(); ++j){
          ...
          if(mult->at(j) < 3)							continue; //Need at least three particles in the jet

          TString histName = "_" + type + TString::Format("_eta-%d_pt-%d_rho-%d", etaBin, ptBin, rhoBin);
          plots["axis2"   + histName]->Fill(-std::log(axis2->at(j)));
          plots["ptD"     + histName]->Fill(ptD->at(j));
          plots["mult"    + histName]->Fill(mult->at(j));
          plots["qg_l"    + histName]->Fill(localQG.computeQGLikelihood(       pt->at(j), eta->at(j), rho, {(float) mult->at(j), ptD->at(j), -std::log(axis2->at(j))}));
          plots["axis2_l" + histName]->Fill(localQG.computeQGLikelihood(       pt->at(j), eta->at(j), rho, {-1, -1, -std::log(axis2->at(j))}));
          plots["ptD_l"   + histName]->Fill(localQG.computeQGLikelihood(       pt->at(j), eta->at(j), rho, {-1, ptD->at(j)}));
          plots["mult_l"  + histName]->Fill(localQG.computeQGLikelihood(       pt->at(j), eta->at(j), rho, {(float) mult->at(j)}));
          plots["qg_c"    + histName]->Fill(localQG_cdf.computeQGLikelihoodCDF(pt->at(j), eta->at(j), rho, {(float) mult->at(j), ptD->at(j), -std::log(axis2->at(j))}));
          plots["axis2_c" + histName]->Fill(localQG_cdf.computeQGLikelihoodCDF(pt->at(j), eta->at(j), rho, {-1, -1, -std::log(axis2->at(j))}));
          plots["ptD_c"   + histName]->Fill(localQG_cdf.computeQGLikelihoodCDF(pt->at(j), eta->at(j), rho, {-1, ptD->at(j)}));
          plots["mult_c"  + histName]->Fill(localQG_cdf.computeQGLikelihoodCDF(pt->at(j), eta->at(j), rho, {(float) mult->at(j)}));
        }
      }
