...
//std::vector<TString> files	= {"QCD_Pt-15to3000_Tune4C_Flat_13TeV_pythia8_S14"};
  ...
  for(TString file : files){
    for(TString jetType : jetTypes){
      ...
      QGLikelihoodCalculator localQG("../data/pdfQG_" + jetType + "_13TeV_testWithVarTransform.root");
      QGLikelihoodCalculator localQG_cdf("../data/pdfQG_" + jetType + "_fineBinning_13TeV_testWithVarTransform.root");
      QGLikelihoodCalculator localQG2("../data/pdfQG_" + jetType + "_13TeV_testWithVarTransform.root", true);
      QGLikelihoodCalculator localQG2_cdf("../data/pdfQG_" + jetType + "_fineBinning_13TeV_testWithVarTransform.root", true);
      QGLikelihoodCalculator localQG3("../data/pdfQG_" + jetType + "_13TeV_testWithVarTransform.root", false, true);
      QGLikelihoodCalculator localQG3_cdf("../data/pdfQG_" + jetType + "_fineBinning_13TeV_testWithVarTransform.root", false, true);
      ...
      for(TString binName : bins.getAllBinNames()){
        for(TString type : {"quark","gluon"}){
          ...
          for(TString var : {"qg","qg2","qg3","axis2","ptD","mult"}){
...
//      plots["qg2_l"   + histName]->Fill(localQG.computeQGLikelihood(        t.pt, t.eta, t.rho, {(float) t.mult, -1, t.axis2}),	t.weight);
        plots["qg2_l"   + histName]->Fill(localQG3.computeQGLikelihood(       t.pt, t.eta, t.rho, {(float) t.mult, t.ptD, t.axis2}),	t.weight);
        plots["qg3_l"   + histName]->Fill(localQG2.computeQGLikelihood(       t.pt, t.eta, t.rho, {t.axis2, t.ptD, (float) t.mult}),	t.weight);	// Decorrelation takes other order
//        plots["qg4_l"   + histName]->Fill(localQG2.computeQGLikelihood(       t.pt, t.eta, t.rho, {t.axis2, (float) t.mult}),		t.weight);
...
//      plots["qg2_c"   + histName]->Fill(localQG_cdf.computeQGLikelihoodCDF( t.pt, t.eta, t.rho, {(float) t.mult, -1, t.axis2}), 	t.weight);
        plots["qg2_c"   + histName]->Fill(localQG3_cdf.computeQGLikelihoodCDF(t.pt, t.eta, t.rho, {(float) t.mult, t.ptD, t.axis2}), 	t.weight);
        plots["qg3_c"   + histName]->Fill(localQG2_cdf.computeQGLikelihoodCDF(t.pt, t.eta, t.rho, {t.axis2, t.ptD, (float) t.mult}), 	t.weight);
//        plots["qg4_c"   + histName]->Fill(localQG2_cdf.computeQGLikelihoodCDF(t.pt, t.eta, t.rho, {t.axis2, (float) t.mult}), 	t.weight);
      ...
      for(auto& plot : plots){
        ...
        for(TString var : {"qg","qg2","qg3","axis2","ptD","mult"}){
          for(TString type : {"_l","_c",""}){
            ...
            if(type == "_l")  	 	roc[var+type]->SetLineColor(var == "qg4" ? 42 : (var == "qg3" ? 46 : (var == "qg2" ? 30 : (var == "qg"? kGray+1 : (var == "axis2"? kGreen+4 : (var == "ptD"? kMagenta+4 : kRed))))));
            else if(type == "_c")  	roc[var+type]->SetLineColor(var == "qg4" ? 28 : (var == "qg3" ? 39 : (var == "qg2" ? 3 : (var == "qg"? kBlack : (var == "axis2"? kYellow : (var == "ptD"? kAzure+10 : kOrange))))));
            else		 	roc[var+type]->SetLineColor(var == "axis2"? kGreen+4 : (var == "ptD"? kMagenta+4 : kRed));
//	    roc[var+type]->SetLineWidth(type == "_l" ? 3 : 1);
	    roc[var+type]->SetLineWidth(type == "_l" ? 1.5 : 0.5);
            ...
            if(var == "qg2") 	entryName += " (var weighted)";
            if(var == "qg3") 	entryName += " (uncorrelated)";
