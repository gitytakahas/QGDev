...
parser.add_option("-i","--input",dest="input",type="string",help="input pset",default="qgMiniTupleForMiniAOD_cfg.py");
...
for idx0,fl in enumerate(fileChunks):
	...
	mylen += len(fl)
	psetFileName = "qgMiniTupleForMiniAOD_%d.py"%(idx)
	...
	if opts.put != "":
		...
		print >> sh, '[ "${EXIT}" == "0" ] && { ' + EOS2 + " mkdir " + "/eos/cms"+opts.put +'  ; ' + copyCmd + ' ${WORKDIR}/qgMiniTuple.root ' + opts.put + '/qgMiniTuple_%(idx)d.root  && touch sub_%(idx)d.done || echo "cmsStage fail" > sub_%(idx)d.fail; }'%{'idx':idx}
	else:
		print >> sh, '[ "${EXIT}" == "0" ] && { cp ${WORKDIR}/qgMiniTuple.root ./qgMiniTuple_%(idx)d.root && touch sub_%(idx)d.done || echo "cp fail" > sub_%(idx)d.fail ; }'%{'idx':idx}
