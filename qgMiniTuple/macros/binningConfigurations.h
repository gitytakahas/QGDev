...
binClass getSmallEtaBinning(){
  ...
  for(int i=10; i < bins.getNBins("pt"); ++i) bins.setLink("eta7_pt9_rho0_aj0",  TString("eta7_pt") + i + "_rho0_aj0");
  for(int i=14; i < bins.getNBins("pt"); ++i) bins.setLink("eta6_pt13_rho0_aj0", TString("eta6_pt") + i + "_rho0_aj0");
  for(int i=15; i < bins.getNBins("pt"); ++i) bins.setLink("eta5_pt14_rho0_aj0", TString("eta5_pt") + i + "_rho0_aj0");
  for(int i=17; i < bins.getNBins("pt"); ++i) bins.setLink("eta4_pt16_rho0_aj0", TString("eta4_pt") + i + "_rho0_aj0");
  for(int i=18; i < bins.getNBins("pt"); ++i) bins.setLink("eta3_pt17_rho0_aj0", TString("eta3_pt") + i + "_rho0_aj0");
  for(int i=19; i < bins.getNBins("pt"); ++i) bins.setLink("eta2_pt18_rho0_aj0", TString("eta2_pt") + i + "_rho0_aj0");
  for(int i=20; i < bins.getNBins("pt"); ++i) bins.setLink("eta1_pt19_rho0_aj0", TString("eta1_pt") + i + "_rho0_aj0");
...
binClass getV2Binning(){														// Same as V1 + additional jets binning
  binClass bins;
  bins.setBinRange("eta", 	"#eta",			{0,1.3,1.5,1.8,2.1,2.4,2.7,3,4.7});
  bins.setBinRange("pt" , 	"p_{T}",		bins.getBins(20, 20, 2000, true, {6500}));
  bins.setBinRange("rho",	"#rho",			{0,15,20,25,30,9999});
...
binClass getV1Binning_(){														// V1 binning in compatibility modus with V2
  binClass bins;
  bins.setBinRange("eta", 	"#eta",			{0,1.3,1.5,1.8,2.1,2.4,2.7,3,4.7});
  bins.setBinRange("pt" , 	"p_{T}",		bins.getBins(20, 20, 2000, true, {6500}));
  bins.setBinRange("rho",	"#rho",			{0,15,20,25,30,9999});
