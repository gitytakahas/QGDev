...
///td::vector<float> xAxisBins	= bins.getBins(40, 0, 4, false, {4.2,4.4,4.7});		TString xVar = "#eta";		bool xLog = false;
//std::vector<float> yAxisBins	= bins.getBins(20, 20, 2000, true, {6500});		TString yVar = "p_{T}"; 	bool yLog = true;
  ...
  for(TString file : files){
    for(TString jetType : jetTypes){
      ...
      float& xAxisVar = t.rho;							// Set x and y-axis variable
	...
	if(!t.balanced) 	continue;										// Take only two leading jets with pt3 < 0.15*(pt1+pt2)
      ...
      for(auto& plot : plots){
        ...
        if(extraBinInformation){
          ...
          kurtosisInBin->Reset();
          skewnessInBin->Reset();
          for(int i = 1; i < xAxisBins.size()-1 + 1; ++i){
            for(int j = 1; j < yAxisBins.size()-1 + 1; ++j){
